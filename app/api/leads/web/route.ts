...
function readToken(req: Request) {
  ...
  // 3) Form field "webhook_token" (נבדוק אחרי שנקרא גוף טקסט, אם צריך)
  return null;
}

async function parseBody(raw: string, contentType: string | null) {
...
export async function POST(req: Request) {
  ...
  // קרא גוף גולמי (חשוב לפני פרסינג)
  const raw = await req.text();
  const contentType = req.headers.get('content-type');
  const body = await parseBody(raw, contentType);

  // בדיקת TOKEN: header/query/body
  const token =
    readToken(req) || (typeof body.webhook_token === 'string' ? body.webhook_token : null);

  if (token !== secretToken) {
    return NextResponse.json({ ok: false, error: 'unauthorized' }, { status: 401 });
  }

  // ----- מכאן המשך הלוגיקה הקיימת שלך -----
  // ... מיפוי שדות, UTM, שמירה ל-Supabase וכו'
